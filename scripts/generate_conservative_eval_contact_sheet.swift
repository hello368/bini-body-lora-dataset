...
let evalDir = root.appendingPathComponent("evaluation/bk_bini_body_v1_conservative")
let outputDir = root.appendingPathComponent("contact_sheets")
try FileManager.default.createDirectory(at: outputDir, withIntermediateDirectories: true)

let files = (try? FileManager.default.contentsOfDirectory(at: evalDir, includingPropertiesForKeys: nil))?
    .filter { $0.lastPathComponent.hasPrefix("bk_bini_body_v1_conservative_eval_") && $0.pathExtension.lowercased() == "png" }
...
func saveJPEG(_ image: NSImage, to url: URL) throws {
    guard let tiff = image.tiffRepresentation,
        ...
        throw NSError(domain: "ConservativeContactSheet", code: 1, userInfo: [NSLocalizedDescriptionKey: "Unable to encode JPEG"])
    }
...
let output = outputDir.appendingPathComponent("bk_bini_body_v1_conservative_eval.jpg")
if FileManager.default.fileExists(atPath: output.path) {
    try FileManager.default.removeItem(at: output)
}
try saveJPEG(canvas, to: output)
print("Wrote \(output.path)")
print("Evaluation images: \(files.count)")
