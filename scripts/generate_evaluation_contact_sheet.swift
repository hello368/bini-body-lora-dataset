...
let evalDir = root.appendingPathComponent("evaluation/body_lora_v1")
...
let files = (try? FileManager.default.contentsOfDirectory(at: evalDir, includingPropertiesForKeys: nil))?
    .filter { $0.lastPathComponent.hasPrefix("body_lora_v1_eval_") && $0.pathExtension.lowercased() == "png" }
...
func saveJPEG(_ image: NSImage, to url: URL) throws {
    guard let tiff = image.tiffRepresentation,
        ...
        throw NSError(domain: "EvaluationContactSheet", code: 1, userInfo: [NSLocalizedDescriptionKey: "Unable to encode JPEG"])
    }
...
let output = outputDir.appendingPathComponent("body_lora_v1_evaluation_sheet.jpg")
